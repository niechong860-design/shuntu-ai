...
import { Download, Copy, Maximize2, Sparkles, ArrowUpRight, Heart, Image as ImageIcon } from "lucide-react";
...
const META = [
  ...
  { model: "MJ V6", prompt: "Cyberpunk portrait with holographic green visor", time: "6m" },
  { model: "Flux.1 Pro", prompt: "Brutalist concrete architecture, stormy sky", time: "14m" },
  { model: "SDXL Turbo", prompt: "Liquid metal sculpture, iridescent chrome", time: "1h" },
  { model: "MJ V6", prompt: "Bioluminescent jellyfish, deep ocean emerald", time: "2h" },
  { model: "Nano Banana", prompt: "Vintage analog synthesizer, dark photography", time: "3h" },
];
...
export function Canvas({ generating, heroIndex }: Props) {
  ...
  return (
    <main className="flex h-screen flex-col overflow-hidden bg-background/60 p-5">
      {/* Top status bar */}
        ...
        {generating ? (
          ...
          <>
            ...
            <div className="absolute left-4 top-4 flex items-center gap-2">
              ...
              <span className="glass rounded-full px-2.5 py-1 font-mono text-[10px] text-foreground/80">2048 × 2048</span>
            </div>
            ...
            <div className="absolute inset-x-4 bottom-4">
              <div className="glass-elevated rounded-xl p-3">
                <div className="text-[9px] uppercase tracking-wider text-muted-foreground">Prompt</div>
                <p className="mt-1 line-clamp-2 text-xs leading-relaxed text-foreground/90">{heroMeta.prompt}</p>
              </div>
            </div>
          </>
        )}
      ...
      <div className="mt-4">
        <div className="mb-2 flex items-center justify-between">
          <div className="text-[10px] font-medium uppercase tracking-[0.16em] text-muted-foreground">Recent</div>
          <button className="text-[10px] text-muted-foreground transition-colors hover:text-foreground">View all →</button>
        </div>
        <div className="grid grid-cols-6 gap-2.5">
          {IMAGES.map((src, i) => (
            <button
              ...
              className="group relative aspect-square overflow-hidden rounded-xl border border-border bg-surface transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-glow"
            >
              ...
              <div className="absolute inset-x-1.5 bottom-1.5 flex items-center justify-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                <Mini><Maximize2 className="h-2.5 w-2.5" /></Mini>
                <Mini><Download className="h-2.5 w-2.5" /></Mini>
                <Mini><Copy className="h-2.5 w-2.5" /></Mini>
                <Mini><ArrowUpRight className="h-2.5 w-2.5" /></Mini>
              </div>
            </button>
          ))}
        </div>
...
function HeroAction({ children, ...rest }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button {...rest} className="glass flex h-8 w-8 items-center justify-center rounded-full text-foreground/90 transition-all hover:bg-primary/20 hover:text-primary">
      {children}
    </button>
  );
}
function Mini({ children }: { children: React.ReactNode }) {
  return (
    <span className="glass flex h-6 w-6 items-center justify-center rounded-md text-foreground/90 transition-colors hover:bg-primary/20 hover:text-primary">
      {children}
    </span>
  );
}
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-gradient-to-br from-surface to-surface-elevated">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
        ...
        <div className="text-[11px] text-muted-foreground">Diffusion model is hard at work</div>
    ...
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-6 backdrop-blur-xl animate-[fade-in_0.2s_ease-out]">
      <div onClick={(e) => e.stopPropagation()} className="glass-elevated relative flex max-h-[90vh] w-full max-w-5xl gap-4 overflow-hidden rounded-2xl p-2">
        ...
        <div className="flex w-72 flex-col p-4">
          ...
          <div className="mt-4 text-[10px] uppercase tracking-wider text-muted-foreground">Prompt</div>
          <p className="mt-2 text-sm leading-relaxed">{meta.prompt}</p>
